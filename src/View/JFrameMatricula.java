...
public class JFrameMatricula extends javax.swing.JFrame {
    ...
    void createMatricula() {
        ...
        if (tabelaCurso.getSelectedRowCount() == 0 || tabelaAluno.getSelectedRowCount() == 0) {
            ...
            try {
                ps = Conexao.conexao().prepareStatement("INSERT INTO matricula (idAluno, idCurso) VALUES (?,?)");
    ...
    void carregarDados() {
        ...
        try {
            ps = Conexao.conexao().prepareStatement("SELECT * FROM matricula");
            rs = ps.executeQuery();
    ...
    void deleteMatricula() {
        try {
            if (tabelaMatricula.getSelectedRowCount() == 0) {
                ...
                ps = Conexao.conexao().prepareStatement("DELETE FROM matricula WHERE idMatricula=?");
    ...
    void updateMatricula() {
        try {
            if (tabelaMatricula.getSelectedRowCount() == 0) {
                ...
                ps = Conexao.conexao().prepareStatement("UPDATE matricula SET idAluno=?, idCurso=? WHERE idMatricula=?");
